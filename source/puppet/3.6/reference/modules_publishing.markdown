...
* `types` --- Resource type documentation generated by the puppet module tool as part of `puppet module build`. DO NOT MODIFY. **We recommend removing this field from your source code and not tracking it in your version control system.**
* `checksums`--- File checksums generated by the puppet module tool as part of `puppet module build`. DO NOT MODIFY. **We recommend removing this field from your source code and not tracking it in your version control system.**

To remove these fields from your source code: 
1. When you are ready to publish your module, run `puppet module build` as normal.
2. Navigate to the pkg/ subdirectory and into the copy of the module (it will be the only other thing in the directory beside the tarball).
3. Open the metadata.json in your editor. 
4. Delete the `types` and `checksums` field at the end of the file.
5. Copy the metadata.json.
6. Navigate out of the /pkg subdirectory and back to your source module directory. 
7. Paste the metadata.json, overwriting the one in your source directory.
...
***How do you know if your metadata.json is partial or complete?** For your metadata.json to be complete, it must contain all the metadata fields listed above.
...
You will receive the deprecation warning, but the PMT will build you a complete metadata.json file using the information in your Modulefile. The PMT will place the metadata.json in both your build directory and root module directory. Then delete the Modulefile, as it is deprecated. *We recommend [removing the deprecated fields from your source metadata.json](#deprecated).

###Modulefile and metadata.json

If you have a complete metadata.json, the PMT will merge your Modulefile and metadata.json files. 

Run the build command.

    # puppet module build <MODULE DIRECTORY>
    
You will receive a deprecation warning. The PMT will merge your Modulefile and metadata.json file into a single metadata.json. To avoid future warnings, you need to copy the metadata.json from the /pkg directory within the build directory and place it in your module's root directory. Then delete the Modulefile, as it is deprecated. *We recommend [removing the deprecated fields from your source metadata.json](#deprecated).
